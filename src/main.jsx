...
// ============================================================
async function scanReceipt(b64){const mt=b64.startsWith('data:image/png')?'image/png':'image/jpeg';const d=b64.split(',')[1];const r=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,messages:[{role:'user',content:[{type:'image',source:{type:'base64',media_type:mt,data:d}},{type:'text',text:'Analyze this receipt. Return ONLY valid JSON:\n{"vendor":"store","description":"brief desc","amount":0.00,"category":"one of: Materials, Labor, Equipment, Permits, Subcontractor, Transportation, Safety, Fuel, Rental, Other","date":"YYYY-MM-DD or empty"}'}]}]})});const data=await r.json();const txt=data.content.map(i=>i.text||'').join('');return JSON.parse(txt.replace(/```json|```/g,'').trim());}
async function fetchWeather(loc){try{const r=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,tools:[{type:'web_search_20250305',name:'web_search'}],messages:[{role:'user',content:`Today's weather for ${loc}? Return ONLY JSON: {"temp_f":0,"condition":"","humidity":0,"wind_mph":0,"high_f":0,"low_f":0,"location":"","date":"YYYY-MM-DD"}`}]})});const data=await r.json();const txt=data.content.filter(i=>i.type==='text').map(i=>i.text).join('');const m=txt.match(/\{[\s\S]*?\}/);return m?JSON.parse(m[0]):null;}catch{return null;}}
...
function App() {
  ...
  const addHoursEntry = async () => {
    ...
    const hrs = form.manHrs ? parseFloat(form.manHrs) : (form.startTime && form.endTime ? calcH(form.startTime, form.endTime) : 0);
    if (hrs <= 0) return; setSaving(true);
    const emp = employees.find(e => e.id === form.selEmpId);
    try {
      await db.insert('hours', { project_id: active, employee_name: name, classification: emp?.classification || form.empCls || '', hours: form.manHrs ? parseFloat(form.manHrs) : hrs, start_time: form.manHrs ? '' : (form.startTime || ''), end_time: form.manHrs ? '' : (form.endTime || ''), date: form.date || today(), notes: form.notes || '' });
      ...
      <div className="page">
        <div className="header">
          ...
          <div className="header__actions">
            <div className="user-bar"><span className="user-bar__email">{user?.email}</span><button className="user-bar__logout" onClick={handleLogout}>Logout</button></div>
          </div>
        </div>
        ...
        {projects.length > 0 && <div className="stats-grid">
          {[{l:'Projects',v:projects.length,c:'#f5a623'},{l:'Employees',v:employees.length,c:'#ab47bc'},{l:'Total Hours',v:projects.reduce((s,p)=>s+p.hours.reduce((a,h)=>a+Number(h.hours),0),0).toFixed(1),c:'#4fc3f7'},{l:'Expenses',v:fmtCur(projects.reduce((s,p)=>s+p.expenses.reduce((a,e)=>a+Number(e.amount),0),0)),c:'#66bb6a'},{l:'Open Tasks',v:projects.reduce((s,p)=>s+p.tasks.filter(t=>t.status!=='Done').length,0),c:'#ef5350'}].map((s,i)=><div key={i} className="stat-card"><div className="stat-card__label">{s.l}</div><div className="stat-card__value" style={{color:s.c}}>{s.v}</div></div>)}
        </div>}
  ...
  return (
    <div className="app">
      ...
      <div className="page">
        ...
        {/* HOURS */}
        {tab==='hours'&&<div>
          <div className="section-header"><div><h3 className="section-title">Employee Hours</h3><p className="section-sub">Total: <strong style={{color:'#4fc3f7'}}>{proj.hours.reduce((a,h)=>a+Number(h.hours),0).toFixed(1)} hrs</strong></p></div><Btn onClick={()=>{setForm({date:today(),startTime:'07:00',endTime:'15:30'});setModal('addHours');}}><span style={{display:'flex',alignItems:'center',gap:6}}><IC.Plus/> Log Hours</span></Btn></div>
        ...
        {/* EXPENSES */}
        {tab==='expenses'&&<div>
          <div className="section-header"><div><h3 className="section-title">Expenses</h3><p className="section-sub">Total: <strong style={{color:'#66bb6a'}}>{fmtCur(proj.expenses.reduce((a,e)=>a+Number(e.amount),0))}</strong></p></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><Btn2 onClick={()=>rcptRef.current?.click()} style={{display:'flex',alignItems:'center',gap:6,padding:'10px 14px'}}><IC.Scan/> Scan</Btn2><input ref={rcptRef} type="file" accept="image/*" capture="environment" onChange={handleReceipt} style={{display:'none'}}/><Btn onClick={()=>{setForm({date:today(),category:'Materials'});setModal('addExpense');}}><span style={{display:'flex',alignItems:'center',gap:6}}><IC.Plus/> Add</span></Btn></div></div>
