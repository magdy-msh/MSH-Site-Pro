...
function App() {
  ...
  const [tsUser, setTsUser] = useState(null); // for admin viewing other users
  const [tsWeekDt, setTsWeekDt] = useState(today());
    ...
    try {
      ...
      const full = await Promise.all(prjs.map(async p => {
        const [photos, hours, expenses, tasks, notes, files] = await Promise.all([
          ...
          db.query('files', { select: 'id,project_id,name,file_type,file_size,folder,uploaded_by,date,created_at', filter: `project_id=eq.${p.id}`, order: 'created_at.desc' }).catch(() => []),
        ]);
  ...
  const refreshProject = async (projId) => {
    const [photos, hours, expenses, tasks, notes, files] = await Promise.all([
      ...
      db.query('files', { select: 'id,project_id,name,file_type,file_size,folder,uploaded_by,date,created_at', filter: `project_id=eq.${projId}`, order: 'created_at.desc' }).catch(() => []),
    ]);
  ...
  const loadTimesheets = async (userId) => {
    try {
      const uid = userId || db.getUser()?.id;
      if (!uid) return;
      const data = await db.query('timesheets', { filter: `user_id=eq.${uid}`, order: 'date.desc' });
      setTimesheets(data || []);
    } catch (e) { console.error('Timesheet load:', e); }
  };

  const addTimesheet = async () => {
    const user = db.getUser(); if (!user) return;
    const dates = (form.extraDates && form.extraDates.length > 0) ? [form.tsDate || today(), ...form.extraDates] : [form.tsDate || today()];
    const uniqueDates = [...new Set(dates)].filter(Boolean);
    setSaving(true);
    try {
      for (const d of uniqueDates) {
        // Check if entry already exists for this date
        const existing = timesheets.find(t => t.date === d);
        if (existing) {
          await db.update('timesheets', existing.id, { status: form.tsStatus || 'worked', start_time: form.startTime || '', end_time: form.endTime || '', hours: parseFloat(form.tsHours || '8'), notes: form.notes || '' });
        } else {
          await db.insert('timesheets', { user_id: user.id, date: d, status: form.tsStatus || 'worked', start_time: form.startTime || '', end_time: form.endTime || '', hours: parseFloat(form.tsHours || '8'), notes: form.notes || '' });
        }
      }
      await loadTimesheets(tsUser || user.id);
    } catch (e) { alert('Error: ' + e.message); }
    setSaving(false); setModal(null); setForm({});
  };

  const printTimesheet = (entries, userName, dateLabel) => {
    const worked = entries.filter(t => t.status === 'worked');
    const off = entries.filter(t => t.status !== 'worked');
    const totalH = worked.reduce((a, t) => a + Number(t.hours), 0);
    openPrint(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Timesheet - ${userName}</title><style>${PS}</style></head><body><h1>${userName} — Timesheet</h1><p class="meta">${dateLabel} · ${new Date().toLocaleString()}</p><div class="stats"><div><div class="stat-label">Days Worked</div><div class="stat-value">${worked.length}</div></div><div><div class="stat-label">Total Hours</div><div class="stat-value">${totalH.toFixed(1)}h</div></div><div><div class="stat-label">Days Off</div><div class="stat-value">${off.length}</div></div></div><h2>Work Log</h2><table><thead><tr><th>Date</th><th>Status</th><th>Start</th><th>End</th><th>Hours</th><th>Notes</th></tr></thead><tbody>${entries.sort((a, b) => a.date.localeCompare(b.date)).map(t => `<tr><td><b>${fmtDate(t.date)}</b></td><td><span class="badge ${t.status === 'worked' ? 'done' : 'pending'}">${t.status === 'worked' ? 'Worked' : t.status === 'off' ? 'Day Off' : t.status === 'sick' ? 'Sick' : t.status}</span></td><td>${t.start_time ? fmtTime(t.start_time) : '—'}</td><td>${t.end_time ? fmtTime(t.end_time) : '—'}</td><td>${t.status === 'worked' ? t.hours + 'h' : '—'}</td><td>${t.notes || '—'}</td></tr>`).join('')}<tr class="total-row"><td colspan="4">Total Hours</td><td>${totalH.toFixed(1)}h</td><td></td></tr></tbody></table><div class="footer">SitePro · Timesheet · ${userName} · ${dateLabel}</div></body></html>`);
  };
  ...
  // TIMESHEET VIEW
  // ============================================================
  if (showTimesheet && !active) {
    const wr = weekRange(tsWeekDt);
    const isAll = tsUser === 'all';
    const weekEntries = timesheets.filter(t => t.date >= wr.start && t.date <= wr.end);
    const userName = isAll ? 'All Employees' : (user?.user_metadata?.full_name || user?.email || 'My');

    // Group by user for admin "all" view
    const byUser = {};
    if (isAll) {
      weekEntries.forEach(t => {
        const uid = t.user_id;
        if (!byUser[uid]) byUser[uid] = { entries: [], name: '' };
        byUser[uid].entries.push(t);
      });
      // Get names from appUsers
      Object.keys(byUser).forEach(uid => {
        const p = appUsers.find(u => u.id === uid);
        byUser[uid].name = p?.full_name || uid.slice(0, 8);
      });
    }
      ...
      <div className="app">
        ...
        <div className="page">
          <div className="detail-header">
            <button className="btn-back" onClick={()=>setShowTimesheet(false)}><IC.Back/></button>
            <div style={{flex:1}}><h2 className="detail-header__name" style={{color:'#4fc3f7'}}>{isAll ? 'All Timesheets' : 'My Timesheet'}</h2><span className="detail-header__sub">{wr.label}</span></div>
            <Btn2 onClick={()=>{
              const label = wr.label;
              if (isAll) {
                const allEntries = timesheets.filter(t => t.date >= wr.start && t.date <= wr.end);
                // Print all grouped by user
                const byU = {};
                allEntries.forEach(t => { if (!byU[t.user_id]) byU[t.user_id] = []; byU[t.user_id].push(t); });
                const sections = Object.entries(byU).map(([uid, entries]) => {
                  const p = appUsers.find(u => u.id === uid);
                  const name = p?.full_name || uid.slice(0, 8);
                  const worked = entries.filter(t => t.status === 'worked');
                  const totalH = worked.reduce((a, t) => a + Number(t.hours), 0);
                  return `<div class="emp-header"><div><b>${name}</b></div><div><b>${totalH.toFixed(1)}h</b> · ${worked.length} days</div></div><table><thead><tr><th>Date</th><th>Status</th><th>Hours</th><th>Notes</th></tr></thead><tbody>${entries.sort((a, b) => a.date.localeCompare(b.date)).map(t => `<tr><td>${fmtDate(t.date)}</td><td><span class="badge ${t.status === 'worked' ? 'done' : 'pending'}">${t.status}</span></td><td>${t.status === 'worked' ? t.hours + 'h' : '—'}</td><td>${t.notes || '—'}</td></tr>`).join('')}</tbody></table>`;
                }).join('<hr style="margin:20px 0;border:none;border-top:1px solid #ddd">');
                const gt = allEntries.filter(t => t.status === 'worked').reduce((a, t) => a + Number(t.hours), 0);
                openPrint(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>All Timesheets</title><style>${PS}</style></head><body><h1>All Employee Timesheets</h1><p class="meta">${label} · ${new Date().toLocaleString()}</p><div class="stats"><div><div class="stat-label">Employees</div><div class="stat-value">${Object.keys(byU).length}</div></div><div><div class="stat-label">Total Hours</div><div class="stat-value">${gt.toFixed(1)}h</div></div></div>${sections}<div class="footer">SitePro · All Timesheets · ${label}</div></body></html>`);
              } else {
                printTimesheet(weekEntries, userName, label);
              }
            }} style={{padding:'8px 14px',display:'flex',alignItems:'center',gap:6}}><IC.Print/> Print</Btn2>
          </div>

          <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:16}}>
            <button className="btn-secondary" style={{padding:'8px 12px'}} onClick={()=>{const d=new Date(tsWeekDt+'T00:00:00');d.setDate(d.getDate()-7);setTsWeekDt(d.toISOString().split('T')[0]);}}>← Prev</button>
            <Inp label="" type="date" value={tsWeekDt} onChange={e=>setTsWeekDt(e.target.value)}/>
            <button className="btn-secondary" style={{padding:'8px 12px'}} onClick={()=>{const d=new Date(tsWeekDt+'T00:00:00');d.setDate(d.getDate()+7);setTsWeekDt(d.toISOString().split('T')[0]);}}>Next →</button>
            {!isAll&&<Btn onClick={()=>{setForm({tsDate:today(),tsStatus:'worked',startTime:'07:00',endTime:'15:30',tsHours:'8'});setModal('addTimesheet');}} style={{marginLeft:'auto'}}><span style={{display:'flex',alignItems:'center',gap:6}}><IC.Plus/> Log Day</span></Btn>}
          </div>

          {isAll ? (
            Object.keys(byUser).length === 0 ? <div className="empty-state empty-state--sm"><p className="empty-state__title">No entries this week</p></div> :
            <div className="list">{Object.entries(byUser).map(([uid, d]) => {
              const worked = d.entries.filter(t => t.status === 'worked');
              const totalH = worked.reduce((a, t) => a + Number(t.hours), 0);
              return <div key={uid} className="date-folder" onClick={async()=>{setTsUser(uid);await loadTimesheets(uid);}} style={{padding:'14px 16px',background:'var(--bg2)',border:'1px solid var(--brd)',borderRadius:12,marginBottom:8,cursor:'pointer'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div><div style={{fontWeight:700,fontSize:15,color:'#fff'}}>{d.name}</div><div style={{fontSize:12,color:'#888',marginTop:4}}>{worked.length} days worked{d.entries.length > worked.length ? `, ${d.entries.length - worked.length} off` : ''}</div></div>
                  <div className="big-value" style={{color:'#4fc3f7'}}>{totalH.toFixed(1)}h</div>
                </div>
              </div>;
            })}</div>
          ) : (
            ...
            {(()=>{
              ...
              for (let i = 0; i < 7; i++) {
                ...
                const entry = weekEntries.find(t => t.date === ds);
                const dayName = d.toLocaleDateString('en-US', { weekday: 'short' });
                const isToday = ds === today();
                days.push(
                  <div key={ds} className="list-item" style={{background: isToday ? 'var(--bg2)' : undefined, borderLeft: isToday ? '3px solid #4fc3f7' : entry ? '3px solid #66bb6a' : '3px solid #333'}}>
                    <div style={{flex:1}}>
                      <div style={{display:'flex',gap:8,alignItems:'center'}}>
                        <span style={{fontWeight:700,color:'#fff',minWidth:40}}>{dayName}</span>
                        <span className="mono-sm">{fmtDate(ds)}</span>
                        {isToday&&<span style={{background:'#4fc3f7',color:'#111',padding:'1px 6px',borderRadius:4,fontSize:9,fontWeight:700}}>TODAY</span>}
                      </div>
                      {entry ? <div style={{marginTop:4}}>
                        <span className="badge" style={entry.status==='worked'?{color:'#2e7d32',background:'#e8f5e9'}:{color:'#e65100',background:'#fff3e0'}}>{entry.status==='worked'?'Worked':entry.status==='off'?'Day Off':entry.status==='sick'?'Sick':entry.status}</span>
                        {entry.status==='worked'&&<span style={{marginLeft:8,color:'#4fc3f7',fontWeight:600}}>{entry.hours}h</span>}
                        {entry.start_time&&<span style={{marginLeft:8,fontSize:12,color:'#888'}}>{fmtTime(entry.start_time)} – {fmtTime(entry.end_time)}</span>}
                        {entry.notes&&<div style={{fontSize:12,color:'#999',marginTop:2,fontStyle:'italic'}}>{entry.notes}</div>}
                      </div> : <div style={{fontSize:12,color:'#555',marginTop:2}}>No entry</div>}
                    </div>
                    <div style={{display:'flex',gap:6}}>
                      <button className="btn-edit" onClick={()=>{setForm({tsDate:ds,tsStatus:entry?.status||'worked',startTime:entry?.start_time||'07:00',endTime:entry?.end_time||'15:30',tsHours:entry?.hours?.toString()||'8',notes:entry?.notes||'',tsId:entry?.id});setModal('addTimesheet');}}><IC.Edit/></button>
                      {entry&&<BtnDel onClick={async()=>{setSaving(true);try{await db.remove('timesheets',entry.id);await loadTimesheets(tsUser||undefined);}catch(e){alert(e.message);}setSaving(false);}}/>}
                    </div>
                  </div>
                );
              }
            ...
            <div style={{marginTop:12,padding:'12px 16px',background:'var(--bg2)',borderRadius:10,border:'1px solid var(--brd)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <span style={{fontWeight:700,color:'#fff'}}>Week Total</span>
              <span className="big-value" style={{color:'#4fc3f7'}}>{weekEntries.filter(t=>t.status==='worked').reduce((a,t)=>a+Number(t.hours),0).toFixed(1)}h</span>
            </div>
            </>
          )}
        </div>
        ...
        {/* ADD TIMESHEET ENTRY */}
        <Modal open={modal==='addTimesheet'} onClose={()=>{setModal(null);setForm({});}} title={form.tsId?'Edit Day':'Log Work Day'}>
          <Inp label="Date" type="date" value={form.tsDate||today()} onChange={e=>setForm(f=>({...f,tsDate:e.target.value}))}/>
          {!form.tsId&&<div style={{marginTop:-8,marginBottom:8}}>
            ...
            <button className="text-btn" style={{marginTop:6,fontSize:12}} onClick={()=>setForm(f=>({...f,extraDates:[...(f.extraDates||[]),today()]}))}>+ Add another day</button>
          </div>}
          <Sel label="Status" value={form.tsStatus||'worked'} onChange={e=>setForm(f=>({...f,tsStatus:e.target.value}))}><option value="worked">Worked</option><option value="off">Day Off</option><option value="sick">Sick Day</option><option value="holiday">Holiday</option><option value="vacation">Vacation</option></Sel>
          {(form.tsStatus||'worked')==='worked'&&<>
            ...
            <Inp label="Hours" type="number" step="0.25" value={form.tsHours||'8'} onChange={e=>setForm(f=>({...f,tsHours:e.target.value}))}/>
          </>}
          <div className="field"><label className="field__label">Notes</label><textarea className="field__input" rows={3} placeholder="What did you work on today?" value={form.notes||''} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} style={{resize:'vertical'}}/></div>
          <div className="modal__actions"><Btn2 onClick={()=>{setModal(null);setForm({});}}>Cancel</Btn2><Btn onClick={addTimesheet} disabled={saving}>{form.tsId?'Update':'Save'}</Btn></div>
        </Modal>
      </div>
    );
  }
    ...
    <div className="app">
      ...
      <div className="page">
        ...
        <div style={{display:'flex',gap:8,marginBottom:20,flexWrap:'wrap'}}>
          ...
          <Btn2 onClick={async()=>{setShowTimesheet(true);setTsUser(null);await loadTimesheets();}} style={{display:'flex',alignItems:'center',gap:6,padding:'10px 14px'}}><IC.Cal/> My Timesheet</Btn2>
          {isAdmin&&<Btn2 onClick={async()=>{setShowTimesheet(true);setTsUser('all');const data=await db.query('timesheets',{order:'date.desc'}).catch(()=>[]);setTimesheets(data||[]);}} style={{display:'flex',alignItems:'center',gap:6,padding:'10px 14px',borderColor:'#f5a623',color:'#f5a623'}}><IC.Cal/> All Timesheets</Btn2>}
        ...
        {tab==='expenses'&&<div>
          ...
          <div>{(()=>{
            ...
            return Object.entries(byDate).sort((a,b)=>b[0].localeCompare(a[0])).map(([date,items])=>{
              ...
              return <div key={date} style={{marginBottom:12,background:'var(--bg2)',border:'1px solid var(--brd)',borderRadius:12,overflow:'hidden'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 16px',borderBottom:'1px solid var(--brd)',background:'rgba(255,255,255,0.02)'}}>
                  <div style={{fontWeight:700,fontSize:15,color:'#fff'}}>{fmtDate(date)}</div>
                  <div style={{display:'flex',alignItems:'center',gap:10}}>
                    {isAdmin&&<span style={{fontWeight:700,color:'#66bb6a',fontSize:16}}>{fmtCur(total)}</span>}
                    <span style={{fontSize:11,color:'#666'}}>{items.length} item{items.length!==1?'s':''}</span>
                  </div>
                </div>
                {items.map(e=><div key={e.id} style={{display:'flex',alignItems:'center',padding:'10px 16px',borderBottom:'1px solid rgba(255,255,255,0.04)',gap:10}} className={e.scanned?'scanned-row':''}>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}>
                      <span style={{fontWeight:600,color:'#eee',fontSize:14}}>{e.description}</span>
                      {e.scanned&&<span className="scanned-badge">Scanned</span>}
                    </div>
                    <div style={{fontSize:12,color:'#777',marginTop:2}}>
                      <span style={{color:'#f5a623'}}>{e.category}</span>
                      {e.subcategory?<span> / {e.subcategory}</span>:null}
                      {e.vendor?<span> · {e.vendor}</span>:null}
                    </div>
                  </div>
                  <div style={{display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
                    {isAdmin&&<span style={{fontWeight:700,color:'#66bb6a',fontSize:14,fontFamily:'var(--mono)'}}>{fmtCur(e.amount)}</span>}
                    {isAdmin&&<button className="btn-edit" onClick={()=>{setEditExpId(e.id);setForm({description:e.description,amount:e.amount?.toString(),category:e.category,subcategory:e.subcategory||'',vendor:e.vendor,date:e.date});setModal('addExpense');}}><IC.Edit/></button>}
                    {isAdmin&&<BtnDel onClick={()=>delItem('expenses',e.id)}/>}
                  </div>
                </div>)}
              </div>;
            });
        ...
        {tab==='files'&&<div>
          ...
          <div className="list">{(proj.files||[]).map(f=>{
            ...
            return <div key={f.id} className="list-item">
              ...
              <div style={{display:'flex',gap:6}}>
                <button className="btn-secondary" style={{padding:'6px 10px',fontSize:11}} onClick={async()=>{
                  try{
                    const [full]=await db.query('files',{filter:`id=eq.${f.id}`});
                    if(full?.file_data){const a=document.createElement('a');a.href=full.file_data;a.download=f.name;a.click();}
                    else{alert('File data not found');}
