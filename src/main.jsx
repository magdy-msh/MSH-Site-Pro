...
const TABS = [{id:'photos',label:'Photos'},{id:'hours',label:'Hours'},{id:'expenses',label:'Expenses'},{id:'tasks',label:'Tasks'},{id:'notes',label:'Notes'}];
...
function App() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const full = await Promise.all(prjs.map(async p => {
        const [photos, hours, expenses, tasks, notes] = await Promise.all([
        ...
        return { ...p, photos, hours, expenses, tasks, notes: notes || [] };
      }));
      setProjects(full);
      setEmployees(emps);
  ...
  const refreshProject = async (projId) => {
    ...
    setProjects(prev => prev.map(p => p.id === projId ? { ...p, photos, hours, expenses, tasks, notes: notes || [] } : p));
  };
    ...
    <div className="app">
      ...
      <div className="page">
        ...
        {tab==='notes'&&<div>
          ...
          <div className="list">{proj.notes.map(n=><div key={n.id} className="list-item" style={{flexDirection:'column',alignItems:'stretch',gap:6}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div className="mono-sm" style={{color:'#f5a623',fontWeight:600}}>{fmtDate(n.date)}{n.author?` · ${n.author}`:''}</div><div style={{display:'flex',gap:6}}><button className="btn-edit" onClick={()=>{setForm({noteDate:n.date,noteText:n.note,noteId:n.id});setModal('addNote');}}><IC.Edit/></button><BtnDel onClick={()=>delItem('daily_notes',n.id)}/></div></div><div style={{fontSize:14,color:'#ddd',lineHeight:1.5,whiteSpace:'pre-wrap'}}>{n.note}</div></div>)}</div>}
        </div>}
      </div>
      ...
      <Modal open={modal==='reports'} onClose={()=>setModal(null)} title="Reports" wide>
        ...
        <div className="report-list">
          ...
          <button className="report-btn" onClick={()=>{
            // General daily report with notes
            const dl=rptDate?fmtDate(rptDate):'All Dates';
            const fN=rptDate?(proj.notes||[]).filter(n=>n.date===rptDate):(proj.notes||[]);
            const fH=rptDate?proj.hours.filter(h=>h.date===rptDate):proj.hours;
            const fE=rptDate?proj.expenses.filter(e=>e.date===rptDate):proj.expenses;
            const fT=rptDate?proj.tasks.filter(t=>t.date===rptDate):proj.tasks;
